...
class ModelPdPd extends Model {
	...
	public function get_all_gd_current_date($status){
		if ($status) {
			...
			$query = $this->db->query("SELECT c.username, c.wallet,c.status as cstatus, gd.*
			FROM ".DB_PREFIX."customer_get_donation gd JOIN ".DB_PREFIX."customer  c
			ON gd.customer_id = c.customer_id WHERE date(gd.date_added)=CURRENT_DATE AND gd.status = ".$status."");
		return $query->rows;
		}else{
			$query = $this->db->query("SELECT c.username, c.wallet,c.status as cstatus, gd.*
			FROM ".DB_PREFIX."customer_get_donation gd JOIN ".DB_PREFIX."customer  c
			ON gd.customer_id = c.customer_id WHERE date(gd.date_added)=CURRENT_DATE");
		return $query->rows;
		}
