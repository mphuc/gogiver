...
//	lay tong so thanh vien
	...
	public function getPD7Before(){
		...
		$query = $this -> db -> query("
			SELECT A.id ,B.username, A.customer_id , A.amount , A.filled,A.date_added,B.telephone,B.email,B.account_holder,B.account_number
...
$date_added= date('Y-m-d H:i:s') ;
	...
	public function getall_gd_date($start_date,$end_date)
	{
		...
		else
		{
			$query = $this -> db -> query("SELECT A.*,B.username,B.account_holder,B.account_number,B.telephone
			FROM  ".DB_PREFIX."customer_get_donation A LEFT JOIN ".DB_PREFIX."customer B
			 ON B.customer_id = A.customer_id WHERE A.date_added >= '".$start_date." 00:00:00' AND A.date_added <= '".$end_date." 23:59:59' AND A.customer_id NOT IN (SELECT customer_id FROM ". DB_PREFIX . "customer WHERE status = 8 OR status = 10)
			ORDER BY A.date_added DESC
		");
		}
		return $query -> rows;
	...
	public function getPNode_level($customer_id){
		$query = $this -> db -> query("
	...
	public function get_all_customer($limit, $start){
		$query = $this -> db -> query("
			SELECT B.*,A.level
	...
	public function getall_pd_datett($start_date,$end_date)
	{
		if ($start_date == $end_date)
		{
	...
	public function user_lock_repd()
	{
		$query = $this -> db -> query("
			SELECT A.*,B.telephone,B.username, (SELECT username FROM  ".DB_PREFIX."customer WHERE customer_id = B.p_node) as upline
			FROM  ".DB_PREFIX."customer_block_id_gd A INNER JOIN ".DB_PREFIX."customer B ON A.customer_id = B.customer_id WHERE A.status = 1 AND B.status <> 8 AND B.status <> 10 ORDER BY A.date DESC
		");
		return $query -> rows;
	}
	...
	public function get_lock_repd_finish()
	{
		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer_block_id_gd A INNER JOIN ".DB_PREFIX."customer B ON A.customer_id = B.customer_id WHERE A.status = 0 AND B.status <> 10 ORDER BY A.date DESC
		");
		return $query -> row;
	}
	...
	{
		$query = $this -> db -> query("
			SELECT A.*,B.telephone,B.username,B.status, (SELECT username FROM  ".DB_PREFIX."customer WHERE customer_id = B.p_node) as upline
			FROM  ".DB_PREFIX."customer_block_id_gd A INNER JOIN ".DB_PREFIX."customer B ON A.customer_id = B.customer_id WHERE A.status = 0 AND B.status <> 10 ORDER BY A.date DESC
			LIMIT ".$limit."
			OFFSET ".$start."
		");
