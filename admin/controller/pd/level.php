...
error_reporting(-1);
		$this->document->setTitle('IOntach');
		...
		$ts_history = $this -> model_sale_customer -> count_get_all_customer();
		...
		$pagination -> url = $this -> url -> link('pd/level', 'page={page}&token='.$this->session->data['token'].'', 'SSL');

		$data['pagination'] = $pagination -> render();

		$data['get_all_customer'] = $this -> model_sale_customer -> get_all_customer($limit, $start);
		...
		$this->response->setOutput($this->load->view('pd/level.tpl', $data));
	}
	...
	public function get_level($customer_id)
	{
		...
		return count($this -> model_sale_customer -> getPNode_level($customer_id));
	...
	public function getCustomer($customer_id)
	{
		...
		return $this -> model_sale_customer -> getCustomer($customer_id)['username'];
